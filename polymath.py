...
import math
from linear import normalize, vector_sub, vector_sum
from linear import scalar_product, scalar_mul, cross_product, d, magnitude
...
def scale_to_plane_on_normal(point, normal):
    return scalar_mul(
        scalar_product(normal, normal) / scalar_product(point, normal), point)


def find_neighbours(plist, ix):
    min_d = min(d(plist[ix], x) for i, x in enumerate(plist) if i != ix)
    return tuple(i for i, x in enumerate(plist)
                 if min_d - .001 < d(plist[ix], x) < min_d + .001)
    ...
    while len(onlist) < len(nlist):
        ...
        for n_out in nlist:
            if n_in != n_out:
                ...
                prod = cross_product(v_in, v_out)
                if scalar_product(p, prod) < 0 and \
                        (next_p is None or sprod < min_scalar):
                    min_scalar = sprod
                    next_p = n_out
                    # next_prod = prod
        onlist += [next_p]
...
def vertex_info(plist, ix):
    ...
    faces = tuple(find_regular_polygon(
        plist, neighbours[j], ix, neighbours[j - 1])
...
def catalan_face(plist, ix):
    ...
    points = tuple(scale_to_plane_on_normal(
        vector_sum(*f), normalize(plist[ix])) for f in faces_coords)
...
def sign_perms(x, y, z):
    ...
    if z == 0:
        zset = (1,)
    return [(x * sx, y * sy, z * sz) for sx in xset for sy in yset
            for sz in zset]
...
def snub_dodecahedron_points():
    ...
    coords = (
        (2 * a, 2., 2 * b),
        ((a + b / phi + phi), (-a * phi + b + 1 / phi),
         (a / phi + b * phi - 1)),
        ((-a / phi + b * phi + 1), (-a + b / phi - phi),
         (a * phi + b - 1 / phi)),
        ((-a / phi + b * phi - 1), (a - b / phi - phi),
         (a * phi + b + 1 / phi)),
        ((a + b / phi - phi), (a * phi - b + 1 / phi),
