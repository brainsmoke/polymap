...
def slot(a, b, d, w, h, start=None, middle=None, end=None):
    ...
    return [(sx, sy), (sx + wdx, sy + wdy),
            (sx + wdx + hdx, sy + wdy + hdy), (sx + hdx, sy + hdy)]
...
def subdivide(shape, types, angles, thickness, overhang, overcut,
              shape_map=shape_map):
    edges = []
...
def grow(poly, width):
    ...
    for a, b, c in zip(poly[-1:] + poly[:-1], poly, poly[1:] + poly[:1]):
        ...
        points.append((bx + width * mid_x / invdist2,
                       by + width * mid_y / invdist2))
