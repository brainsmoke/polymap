...
def get_regions(filename):
    ...
    for line in open(filename).readlines():
        if line[0] == '#':
            ...
            r.append(tuple(map(
                lambda x: math.radians(float(x)), line.split("\t"))))

    return regions


def get_globe():
    return join_adjacent(mercator_to_globe(
        get_regions("data/24577.dat")), dist=squared_distance)
...
def get_map(filename):
    ...
    for r in open(filename).readlines():
        regions.append(
            tuple(tuple(float(x) for x in p.split(','))
                  for p in r.split('|')))
